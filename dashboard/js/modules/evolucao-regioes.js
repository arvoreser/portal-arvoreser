...
(function(){
  ...
  window.renderEvolucaoRegioes1Mes=function(){
    ...
    const canon=new Map();
    const add=(arr,campo)=>arr.forEach(nome=>{
      const k=String(nome??'').trim().toLowerCase(); if(!k)return;
      if(!canon.has(k))canon.set(k,{label:nome,inicial:0,mes1:0});
      canon.get(k)[campo]++;
    });
    d.pares.forEach(p=>{
      add([...new Set(p.inicial.map(x=>String(x).toLowerCase()))],'inicial');
      add([...new Set(p.mes1.map(x=>String(x).toLowerCase()))],'mes1');
    });
    const itens=[...canon.values()].sort((a,b)=>Math.max(b.inicial,b.mes1)-Math.max(a.inicial,a.mes1)||String(a.label).localeCompare(String(b.label),'pt-BR'));
