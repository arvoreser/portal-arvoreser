...
(function(){
  ...
  function abrirDetalheRegiao(index,datasetIndex){
    const item=itensAtuais[index]; if(!item)return;
  ...
  window.renderEvolucaoRegioes1Mes=function(){
    if(!bloco())return;
    const d=dados(); itensAtuais=montarItens(d);
    ...
    chart=new Chart(document.getElementById('evRegChart'),{type:'bar',data:{labels:itensAtuais.map(x=>x.label),datasets:[{label:'Avaliação inicial',data:itensAtuais.map(x=>x.inicial.length),backgroundColor:'rgba(253,49,5,.28)',borderColor:'#fd3105',borderWidth:1,borderRadius:5},{label:'1 mês',data:itensAtuais.map(x=>x.mes1.length),backgroundColor:'rgba(23,250,3,.25)',borderColor:'#187900',borderWidth:1,borderRadius:5}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,onClick:(evt,elements)=>{if(elements.length)abrirDetalheRegiao(elements[0].index,elements[0].datasetIndex);},plugins:{legend:{position:'top'},tooltip:{callbacks:{label:c=>`${c.dataset.label}: ${c.raw} colaborador(es)`,afterLabel:()=> 'Clique para ver os nomes'}}},scales:{x:{beginAtZero:true,ticks:{precision:0}},y:{grid:{display:false}}}}});
  };
})();