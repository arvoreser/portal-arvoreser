...
(function(){
  ...
  function nota(v){ const m=texto(v).replace(',','.').match(/\d+(?:\.\d+)?/); if(!m) return 0; const n=Number(m[0]); return Number.isFinite(n)?n:0; }
  function risco(n,dor){ if(n>=7)return '🔴'; if(n>=4)return '🟠'; if(dor)return '🟡'; return '🟢'; }
  function contarRaw(items,campo){ const mapa={}; items.forEach(r=>{ const v=texto(r[campo]); if(v)mapa[v]=(mapa[v]||0)+1; }); return Object.entries(mapa).sort((a,b)=>b[1]-a[1]); }
  function contarNorm(items,campo){ const mapa={}; items.forEach(r=>{ const v=texto(r[campo]); if(v&&v!=='Não informado')mapa[v]=(mapa[v]||0)+1; }); return Object.entries(mapa).sort((a,b)=>b[1]-a[1]); }
